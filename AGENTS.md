...
- There are TWO entry points to sharing, on purpose: the long press on the card, and a "Compartir con…" row inside `ReminderEditorDialog`, right under "Recordar en…". The long press alone was undiscoverable. The editor row is hidden for reminders owned by someone else.
- Sharing from the EDIT dialog writes straight to the document, and the `ShareReminderDialog` opens ON TOP of the editor instead of replacing it: the editor stays composed, so text and due date typed but not yet saved survive.
- Sharing from the CREATE dialog cannot write anything yet, because `contributors` needs an existing document. The selection lives in `RemindersScreen` (`newReminderShareWith`) and `createReminder(text, due, shareWith)` applies it with one `shareReminder` call per friend AFTER the create succeeds. A failure there does not undo the creation: the reminder is saved and only the reparto is reported, through `ReminderSharingFeedback(isSuccess = false)`.
- `ShareReminderDialog` takes a NULLABLE `reminderText` and a `dismissLabel`: with a real document it shows the text and "Cancelar", and over the pre-create selection it shows neither text nor a cancel, just "Listo". The same composable serves both because it was already driven entirely by parameters.
- `loadShareableFriendsForNewReminder()` exists alongside `loadShareableFriends(reminder)` because there is no document to filter contributors against yet; it offers every friend and the screen filters out what is already selected.
